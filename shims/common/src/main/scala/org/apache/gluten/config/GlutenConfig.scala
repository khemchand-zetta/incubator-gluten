...
object GlutenConfig {
  ...
  // Configurations for constroling the broadcast hash shared table
  val COLUMNAR_BROADCAST_JOIN_HASH_TABLE_SHARING = buildConf("spark.gluten.sql.columnar.velox.broadcastJoin.hashTableSharing")
    .internal()
    .doc("Enable sharing of broadcast join hash tables between tasks in the same executor")
    .booleanConf
    .createWithDefault(true)

  val COLUMNAR_BROADCAST_JOIN_HASH_TABLE_EXPIRE_TIME = buildConf("spark.gluten.sql.columnar.velox.broadcast.expire.seconds")
    .internal()
    .doc("Time in seconds after which a cached broadcast hash table will expire")
    .intConf
